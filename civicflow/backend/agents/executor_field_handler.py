...
def generate_autofill_script(session_dict: dict, script_path: str) -> str:
    ...
    b.add("# Check for CAPTCHA before submission")
    ...
    b.add("# Poll for genuine completion while browser remains open")
    b.add("while True:")
    b.indent()
    b.add("if page.is_closed():")
    b.indent()
    ...
    b.indent()
    b.add('print("EVENT:captcha_completed:Resume signal received", flush=True)')
    b.add("break")
    b.dedent()
    b.add("try:")
    b.indent()
    b.add("is_solved = await page.evaluate('''() => {")
    b.indent()
    b.add("const cb = document.getElementById('captcha_checkbox');")
    b.add("if (cb && cb.checked) return true;")
    b.add("if (window.grecaptcha && typeof window.grecaptcha.getResponse === 'function') {")
    b.indent()
    b.add("const resp = window.grecaptcha.getResponse();")
    b.add("if (resp && resp.length > 0) return true;")
    b.dedent()
    b.add("}")
    b.add("if (window.hcaptcha && typeof window.hcaptcha.getResponse === 'function') {")
    b.indent()
    b.add("const resp = window.hcaptcha.getResponse();")
    b.add("if (resp && resp.length > 0) return true;")
    b.dedent()
    b.add("}")
    b.add("const container = document.getElementById('recaptcha_checkpoint_container') || document.querySelector('.g-recaptcha');")
    b.add("if (container && (container.getAttribute('data-captcha-verified') === 'true' || container.classList.contains('verified'))) return true;")
    b.add("return false;")
    b.dedent()
    b.add("}''')")
    b.add("if is_solved:")
    b.indent()
    b.add('print("EVENT:captcha_completed:Human verification completed in browser", flush=True)')
    b.add("break")
    b.dedent()
    b.dedent()
    b.add("except Exception as e:")
    b.indent()
    b.add("err_str = str(e)")
    b.add('if "closed" in err_str.lower():')
    b.indent()
    b.add('print("EVENT:interrupted:Browser window closed by user", flush=True)')
    b.add("return")
    b.dedent()
    ...
    b.indent()
    b.add('fallback_submit = page.locator(\'#submit_application_btn, button[type="submit"], input[type="submit"], button:has-text("Submit Application"), button:has-text("Submit"), [role="button"]:has-text("Submit")\')')
    ...
    b.indent()
    b.add('submit_loc = page.locator(\'#submit_application_btn, button[type="submit"], input[type="submit"], button:has-text("Submit Application"), button:has-text("Submit")\').first')
    b.dedent()
    ...
    b.add("app_id = None")
    b.add("for _ in range(15):")
    b.indent()
    b.add("current_url = page.url")
    b.add("page_text = await page.evaluate('() => document.body.innerText')")
    b.add("has_success_attr = await page.evaluate('() => Boolean(document.querySelector(\"[data-civicflow-success]\"))')")
    b.add("if has_success_attr or 'formResponse' in current_url or 'response has been recorded' in page_text.lower() or 'submitted' in page_text.lower() or 'thank you' in page_text.lower() or 'successful' in page_text.lower():")
    b.indent()
    b.add("verified = True")
    b.add("try:")
    b.indent()
    b.add("app_id_elem = page.locator('#application_id_display')")
    b.add("if await app_id_elem.count() > 0:")
    b.indent()
    b.add("app_id = (await app_id_elem.first.inner_text()).strip()")
    b.dedent()
    b.add("else:")
    b.indent()
    b.add("import re")
    b.add("match = re.search(r'CF-[A-Z0-9-]+', page_text)")
    b.add("if match:")
    b.indent()
    b.add("app_id = match.group(0)")
    b.dedent()
    b.dedent()
    b.dedent()
    b.add("except Exception:")
    ...
    b.add("if app_id:")
    b.indent()
    b.add('print(f"EVENT:submission_complete:{app_id}", flush=True)')
    b.dedent()
    b.add("else:")
    b.indent()
